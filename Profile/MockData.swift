...
class MockData {
    ...
    static func createMockProjects() {
        ...
        MockData.projects.append(XProjects(name: "PiWatch", platform: "iOS", desc: "Advanced Mobile Systems IoT project using Raspberry Pi, motion sensor, Pi camera and a iOS mobile application. On detection of motion, the camera clicks a still image, creates an intrusion eventa and triggers a push notification to the iOS application, informing the user of the event", image: "stockImage"))
        MockData.projects.append(XProjects(name: "Mobile Pain Diary", platform: "Android", desc: "A university project to develop an Android application that uses Google Geocoding and connection to RESTful web services, weather APIs and databases, that helps patients keep a record of their pain levels", image: "stockImage"))
        MockData.projects.append(XProjects(name: "Wish List", platform: "Android", desc: "A university project to develop an Android application that works as a gift registry and does basic web scraping to find prices of the items on the registry", image: "stockImage"))
    ...
    static func createWorkEx() {
        ...
        MockData.workEx.append(XWorkEx(company: "Telstra", location: "Melbourne, Australia", years: "November 2016 - February 2017", role: "Software Developer - Intern", projects: Array(MockData.projects[0...1]), desc: "Created a working demonstration (Android and web application) of Bluetooth and Wi-Fi based indoor location tracking capabilities for internal use at Telstra Labs \nDeveloped the frontend and backend services using Java, AngularJS, HTML, CSS, Python, Node.js, MongoDB and NGINX \nDelivered a technical presentation on the feasibility and use cases of Bluetooth-based Location Services to Chief Technology Office (CTO)", image: "telstra"))
    }
